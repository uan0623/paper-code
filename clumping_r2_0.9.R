...
# 定義資料夾名稱 ####
prefix <- "C:/Peter/repeatSNP_clumping/r2_filter_0.9/"
pops <- c("EUR", "FIN")
LD <- c(0.1, 0.2, 0.3)
...
if(nrow(gt_N_bon)!=0){
  fwrite(gt_N_bon,
         "C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/1_bon.txt",
...
if(nrow(gt_N_fdr)!=0){
  fwrite(gt_N_fdr,
         "C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/1_fdr.txt",
         row.names = F, col.names = T, sep = "\t") 
...
if(nrow(gt_N_qval)!=0){
  fwrite(gt_N_qval,
         "C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/1_qval.txt",
         row.names = F, col.names = T, sep = "\t") 
...
if(nrow(ld_fdr)!=0){
  ...
  fwrite(  ld_fdr[, .(CHR, SNP_hg19, alt=SNP_alt, ref=SNP_ref)],
           "C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/2_fdr.txt",
           row.names = F, col.names = T, sep = "\t") 
...
if(nrow(ld_qval)!=0){
  ...
  fwrite(  ld_qval[, .(CHR, SNP_hg19, alt=SNP_alt, ref=SNP_ref)],
           "C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/2_qval.txt",
           row.names = F, col.names = T, sep = "\t") 
...
if(nrow(ld_bon)!=0){
  ...
  fwrite(  ld_bon[, .(CHR, SNP_hg19, alt=SNP_alt, ref=SNP_ref)],
           "C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/2_bon.txt",
           row.names = F, col.names = T, sep = "\t") 
...
fwrite( most_p[MOST_snp_hg38_finngen_FDR<0.05,
        ...
        "C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/3_fdr.txt",
        row.names = F, col.names = T, sep = "\t") 
...
fwrite( most_p[MOST_snp_hg38_finngen_Bonfi==1,
               .(CHR = MOST_snp_nearest_chr,
                 ...
                 alt = MOST_snp_nearest_alt,
                 ref = MOST_snp_nearest_ref)],
        "C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/3_bon.txt",
        row.names = F, col.names = T, sep = "\t") 
...
prefix <- "C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/"
files <- c("3_bon.txt", "3_fdr.txt")
file_name <- paste0(prefix, files)

files <- c( "3_bon_SNP.txt", "3_fdr_SNP.txt")
prefix <- "C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/trash/"
...
# 定義目錄（建議統一管理）
tmp_dir    <- "C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/trash/tmp_chr"    # 存放 1-22 染色體的小檔案
final_dir  <- "C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome" # 存放合併後的結果
...
for (i in file_name) {
  ...
  # 讀取 BIM
  bim_path <- sprintf("C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/%s%s", prefix, bim_suffix)
  ...
  # --- 步驟 3: 存檔 ---
  output_path <- sprintf("C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/%s_summarySTAT.txt", prefix)
...
for (i in file_name) {
  ...
  for (para in c(0.1, 0.2, 0.3)) {
    ...
    binary_path <- sprintf("C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/%s%s", prefix, bim_suffix)
    summary_path <- sprintf("C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/%s_summarySTAT.txt", prefix)
    
    
    
    # 1000kb 
    final_out <- sprintf("C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/1000kb_LD%s/%s", para, prefix)
    ...
    # 500kb 
    final_out <- sprintf("C:/Peter/repeatSNP_clumping/r2_filter_0.9/EUR/outcome/500kb_LD%s/%s", para, prefix)
...
FIN <- fread("C:/Peter/QN_before_eQTL/r2_filter_0.9/outcome/QN_MixFinngenPval_7_FIN.txt")
...
if(nrow(gt_N_bon)!=0){
  fwrite(gt_N_bon,
         "C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/1_bon.txt",
         row.names = F, col.names = T, sep = "\t") 
...
if(nrow(gt_N_fdr)!=0){
  fwrite(gt_N_fdr,
         "C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/1_fdr.txt",
         row.names = F, col.names = T, sep = "\t") 
...
if(nrow(gt_N_qval)!=0){
  fwrite(gt_N_qval,
         "C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/1_qval.txt",
         row.names = F, col.names = T, sep = "\t") 
...
if(nrow(ld_fdr)!=0){
  ...
  # 統一欄位名稱，並存檔
  fwrite(  ld_fdr[, .(CHR, SNP_hg19, alt=SNP_alt, ref=SNP_ref)],
           "C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/2_fdr.txt",
           row.names = F, col.names = T, sep = "\t") 
}else(message("ld_fdr no snp"))
...
if(nrow(ld_qval)!=0){
  ...
  fwrite(  ld_qval[, .(CHR, SNP_hg19, alt=SNP_alt, ref=SNP_ref)],
           "C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/2_qval.txt",
           row.names = F, col.names = T, sep = "\t") 
...
if(nrow(ld_bon)!=0){
  ...
  fwrite(  ld_bon[, .(CHR, SNP_hg19, alt=SNP_alt, ref=SNP_ref)],
           "C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/2_bon.txt",
           row.names = F, col.names = T, sep = "\t") 
...
fwrite( most_p[MOST_snp_hg38_finngen_FDR<0.05,
               .(CHR = MOST_snp_nearest_chr,
                 ...
                 alt = MOST_snp_nearest_alt,
                 ref = MOST_snp_nearest_ref)],
        "C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/3_fdr.txt",
        row.names = F, col.names = T, sep = "\t") 
...
fwrite( most_p[MOST_snp_hg38_finngen_Bonfi==1,
               .(CHR = MOST_snp_nearest_chr,
                 ...
                 alt = MOST_snp_nearest_alt,
                 ref = MOST_snp_nearest_ref)],
        "C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/3_bon.txt",
        row.names = F, col.names = T, sep = "\t") 
...
prefix <- "C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/"
files <- c("3_bon.txt", "3_fdr.txt")
file_name <- paste0(prefix, files)

files <- c( "3_bon_SNP.txt", "3_fdr_SNP.txt")
prefix <- "C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/trash/"
file_output <- paste0(prefix, files)
...
# 定義目錄（建議統一管理）
tmp_dir    <- "C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/trash/tmp_chr"    # 存放 1-22 染色體的小檔案
final_dir  <- "C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome" # 存放合併後的結果
...
for (i in file_name) {
  ...
  # 讀取 BIM
  bim_path <- sprintf("C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/%s%s", prefix, bim_suffix)
  bim <- fread(bim_path, col.names = c("chr", "hg19_snpID", "genetic_dist", "pos", "minor", "major"))
  ...
  # --- 步驟 3: 存檔 ---
  output_path <- sprintf("C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/%s_summarySTAT.txt", prefix)
  fwrite(final_data, output_path, row.names = FALSE, sep = "\t")
...
for (i in file_name) {
  ...
  for (para in c(0.1, 0.2, 0.3)) {
    # 設 binary file, summary stat 路徑
    binary_path <- sprintf("C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/%s%s", prefix, bim_suffix)
    summary_path <- sprintf("C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/%s_summarySTAT.txt", prefix)
    
    
    
    # 1000kb 
    final_out <- sprintf("C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/1000kb_LD%s/%s", para, prefix)
    cmd_extract <- sprintf('plink --bfile "%s" --clump "%s" --clump-p1 0.99999 --clump-r2 %s --clump-kb 1000 --clump-snp-field SNP --clump-field P --out "%s"',
                           binary_path, summary_path, para, final_out)
    shell(cmd_extract)
    ...
    # 500kb 
    final_out <- sprintf("C:/Peter/repeatSNP_clumping/r2_filter_0.9/FIN/outcome/500kb_LD%s/%s", para, prefix)
    cmd_extract <- sprintf('plink --bfile "%s" --clump "%s" --clump-p1 0.99999 --clump-r2 %s --clump-kb 500 --clump-snp-field SNP --clump-field P --out "%s"',
                           binary_path, summary_path, para, final_out)
    shell(cmd_extract)
  }
