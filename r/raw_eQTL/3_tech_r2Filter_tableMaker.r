...
r2_threshold <- "0.8"
tissue <- "N"
x <- fread(sprintf("C:/Peter/rawData_eQTL/trash/raw_maf_gt_%s_pvalue.txt", tissue),
header = T
)
R2_filter <- fread(sprintf(
"D:/oral_cancer/expression/outcome/multiple_nucleotide_variant/cis_snp_R2_%s.txt",
r2_threshold
),header = T)


x_sub <- x[SNP %in% R2_filter$hg18_snpID,]
cat("rawData_eQTL r2 threshold 0.8 bon denominator %s", nrow(x_sub))
cat("\n")
# 固定基因的
x <- fread(sprintf("C:/Peter/OQN_FIXgene_before_eQTL/trash/OQN_maf_gt_%s_pvalue.txt", tissue),
header = T
)
x_sub <- x[SNP %in% R2_filter$hg18_snpID,]
cat("OQN_FIXgene_before_eQTL r2 threshold 0.8 bon denominator %s", nrow(x_sub))
cat("\n")

# 固定人的
x <- fread(sprintf("C:/Peter/OQN_FIXpeople_before_eQTL/trash/OQN_maf_gt_%s_pvalue.txt", tissue),
header = T
)
x_sub <- x[SNP %in% R2_filter$hg18_snpID,]
cat("OQN_FIXpeople_before_eQTL r2 threshold 0.8 bon denominator %s", nrow(x_sub))
cat("\n")
...
a <- fread("C:/Peter/repeatSNP_clumping_raw/af_clumping_SNPID.txt")
