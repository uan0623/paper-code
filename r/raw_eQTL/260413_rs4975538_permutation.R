...
library(bestNormalize)
  ...
  all_paths <- c(
    "C:/Peter/rs4975538_permutation/outcome/rawData_eQTL/"
  )
  ...
  exp_diff <- fread("C:/Peter/rawData_eQTL/r2_filter_0.8/outcome/raw_exp_different_r2_0.8.txt", header = T)
  ...
  for (part in c("N", "T")) {
     pval_all <- sprintf(
       "C:/Peter/rawData_eQTL/trash/raw_maf_gt_%s_pvalue.txt",
       part
     ) %>% fread()
    ...
    a <- sprintf(
       "C:/Peter/rawData_eQTL/r2_filter_0.8/outcome/raw_%s_FDR_R2_0.8.txt",
       part
     ) %>% fread()
    ...
    fwrite(af_clump_fdr,
      sprintf("C:/Peter/rs4975538_permutation/outcome/rawData_eQTL/rs4975538_%s_eQTL.txt",
       part),
  ...
  # probe cor ----
  exp_N_raw <- fread("C:/Peter/rawData_eQTL/raw_Exp_mulInterval_N.txt", header = T)
  probe_rs4975538 <- fread("C:/Peter/rs4975538_permutation/outcome/rawData_eQTL/rs4975538_N_eQTL.txt")

  probe_N <- unique(probe_rs4975538$PROBE_ID)
  exp_N_raw <- exp_N_raw[PROBE_ID %in% probe_N, ]
  exp_N_raw <- t(exp_N_raw) %>% as.data.table()

  names(exp_N_raw) <- as.character(exp_N_raw[1, ])
  exp_N_raw <- exp_N_raw[-c(1), ]
  exp_N_raw[, (names(exp_N_raw)) := lapply(.SD, as.numeric)]
  setcolorder(exp_N_raw, probe_N)
  ...
  plot_abs_correlation <- function(exp_data, title) {
    ...
    cor_dt[, abs_cor_group := cut(
      ...
      labels = c("|r| < 0.3", "0.3 <= |r| < 0.6", "|r| >= 0.6"),
      right = FALSE
    )]
    ...
    ggplot(cor_dt, aes(x = probe_x, y = probe_y, fill = abs_cor_group)) +
      ...
      scale_fill_manual(
        ...
        labels = c(
          ...
          expression(0.3 <= abs(r) * " < " * 0.6),
          expression(abs(r) >= 0.6)
        ),
        drop = FALSE,
        name = "absolute cor."
      ) +
  ...
  png("C:/Peter/rs4975538_permutation/outcome/rawData_eQTL/rs4975538_asso_probe_correlation.png",
    width = 14, height = 12, units = "in", res = 300
  )
  
  plot_abs_correlation(exp_N_raw, title = "rs4975538 Normal Part eQTL Gene Cor. After raw") 
  # 關閉檔案
  dev.off()


  png("C:/Peter/rs4975538_permutation/outcome/rawData_eQTL/rs4975538_asso_probe_correlation_raw.png",
    width = 14, height = 12, units = "in", res = 300
  )
...
# snp gt data ----
# a <- fread("C:/Peter/rawData_eQTL/cis_snp_gt_maf_chr5.txt")
# a <- a[ID %in% "5:1333830",]
# fwrite(a,
...
for (j in c("N","T")) {
  ...
  exp <- sprintf("C:/Peter/rawData_eQTL/raw_Exp_mulInterval_%s.txt",j) %>% 
    fread()
  ...
  fwrite(final,
         sprintf("C:/Peter/rs4975538_permutation/outcome/rawData_eQTL/raw_Exp_%s_permu.txt",j),
         row.names = F, col.names = T, sep = "\t")
  ...
  full_snpspos = fread("C:/Peter/rawData_eQTL/snp_pos.txt", header = TRUE)
  ...
  gene_T$LoadFile("C:/Peter/rs4975538_permutation/outcome/rawData_eQTL/raw_Exp_T_permu.txt")
  ...
  gene_N$LoadFile("C:/Peter/rs4975538_permutation/outcome/rawData_eQTL/raw_Exp_N_permu.txt")
  ...
  test <- Matrix_eQTL_main(
    ...
    output_file_name.cis = "C:/Peter/rs4975538_permutation/outcome/rawData_eQTL/gt_N_permu.txt",
  ...
  test_1 <- Matrix_eQTL_main(
    ...
    cvrt = SlicedData$new(), # 沒有 covariates
    output_file_name = NULL,
    output_file_name.cis = "C:/Peter/rs4975538_permutation/outcome/rawData_eQTL/gt_T_permu.txt",
    useModel = modelLINEAR,
    errorCovariance = numeric(),
    pvOutputThreshold.cis = 1, #  # cis snp pvalue <= 1
    pvOutputThreshold = 0,
  ...
  for (part in c("N", "T")) {
    df_raw <- sprintf("C:/Peter/rs4975538_permutation/outcome/rawData_eQTL/rs4975538_%s_eQTL.txt", part) %>%
      fread()
    setkey(df_raw, `p-value`)

    gt <- sprintf("C:/Peter/rs4975538_permutation/outcome/rawData_eQTL/gt_%s_permu.txt", part) %>%
      fread()
    ...
    setcolorder(
      ...
      c(
        ...
        "sig_pval_Bonfi", "t_raw", "pval_raw", "pval_raw_BH", "sig_raw", "sig_raw_Bonfi"
      ) 
    )

    fwrite(df_raw,
      sprintf("C:/Peter/rs4975538_permutation/outcome/rawData_eQTL/rs4975538_%s_eQTL.txt", part),
      row.names = F, col.names = T, sep = "\t"
    )
  }
