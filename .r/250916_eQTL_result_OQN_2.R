...
# 2. 每個 probe 加上 cisSNP number，放進 OQN_%s_FDR_R2_%s.txt, OQN_%s_bon_R2_%s.txt 等不同 imputed quality r2 紀錄 differentialy express的檔案 
...
compute_FDR <- function(inputname,R2filter ,outputname, qvalue_type){
  ...
  if(r2_threshold =="no"){
    for (tissue in c("N","T")) {
          compute_FDR(sprintf("C:/Peter/OQN_FIXpeople_before_eQTL/trash/OQN_maf_gt_%s_pvalue.txt", tissue),
                    "D:/oral_cancer/expression/outcome/multiple_nucleotide_variant/cis_hg18_19_snp_MAF.txt",
                    sprintf("C:/Peter/OQN_FIXpeople_before_eQTL/r2_filter_%s/outcome/OQN_maf_gt_%s_pvalue_FDR_R2_%s.txt",
...
gt <- fread("C:/Peter/OQN_FIXpeople_before_eQTL/trash/OQN_maf_gt_N_pvalue.txt")
  ...
  fwrite(data.table(Has_association_probe = probe_cisSNP),
         sprintf("C:/Peter/OQN_FIXpeople_before_eQTL/r2_filter_%s/outcome/Has_association_probe_R2_%s.txt",
...
df <- fread("C:/Peter/OQN_FIXpeople_before_eQTL/outcome/OQN_exp_different.txt",header=T)
file_name <- c("N","T")

  for (file in file_name) {
  
  a <- sprintf("C:/Peter/OQN_FIXpeople_before_eQTL/r2_filter_%s/outcome/OQN_maf_gt_%s_pvalue_FDR_R2_%s.txt", 
  ...
  fwrite(df,
         sprintf("C:/Peter/OQN_FIXpeople_before_eQTL/r2_filter_%s/outcome/OQN_exp_different_r2_%s.txt",
         r2_threshold, r2_threshold),
         row.names = F, col.names = T, sep = "\t")
...
final_cols <- c(
  ...
  "p-value", "FDR", "qvalue", "sig_pval_Bonfi", "t_OQN", "pval_OQN",
...
mix_info <- function(inputname) {
  ...
  fdr[, `:=`(
    ...
    pval_OQN = format(pval_OQN, digits = 4, scientific = TRUE),
    t_OQN = format(t_OQN, digits = 4, scientific = TRUE),
    R2 = round(R2, 4),
...
setnames(pvalue_OQN, "PROBE_ID", "gene2", skip_absent = TRUE)
...
cols_remove <- grep("_sigCis-SNP_number", names(pvalue_OQN), value = TRUE)
pvalue_OQN[, (cols_remove) := NULL]
...
grid[, input := sprintf(
  "C:/Peter/OQN_FIXpeople_before_eQTL/r2_filter_%s/outcome/OQN_maf_gt_%s_pvalue_FDR_R2_%s.txt",
  r2, type, r2
)]
grid[, output := sprintf(
  "C:/Peter/OQN_FIXpeople_before_eQTL/r2_filter_%s/outcome/OQN_%s_FDR_R2_%s.txt",
  r2, type, r2
)]
grid[, bon_output := sprintf(
  "C:/Peter/OQN_FIXpeople_before_eQTL/r2_filter_%s/outcome/OQN_%s_bon_R2_%s.txt",
  r2, type, r2
)]
