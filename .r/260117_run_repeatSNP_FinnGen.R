...
for (r2_threshold in c( "0.6","0.7", "0.8","0.9","no")) {
  ...
  source("C:/Peter/vscode/.r/repeatSNP_FinnGen.R", local = env)
...
data_1 <- fread(
  sprintf("C:/Peter/OQN_FIXpeople_before_eQTL/r2_filter_%s/outcome/OQN_MixFinngenPval_7_EUR.txt",
          r2_threshold) 
)
...
tmp_dir <- sprintf("C:/Peter/repeatSNP_clumping_FIXpeople/r2_filter_%s/outcome", r2_threshold)
if(!dir.exists(tmp_dir)) dir.create(tmp_dir, recursive = TRUE)

png(
  sprintf("C:/Peter/repeatSNP_clumping_FIXpeople/r2_filter_%s/outcome/BHplot_EUR.png", r2_threshold),
  width = 2000,
  height = 1000,
  res = 200
)
...
data_N_r2  <- fread("C:/Peter/OQN_FIXpeople_before_eQTL/trash/OQN_maf_gt_N_pvalue.txt",header = T)
# R2 filter
data_N_r2  <- data_N_r2[SNP %in% R2_filter$hg18_snpID,]


data_T_r2  <- fread("C:/Peter/OQN_FIXpeople_before_eQTL/trash/OQN_maf_gt_T_pvalue.txt",header = T)
...
# 0.8 BH plot ----
png("C:/Peter/repeatSNP_clumping_FIXpeople/r2_filter_0.8/outcome/BHplot_r2_0.8_original.png", width = 2000, height = 1000, res = 200)
par(mfrow = c(1, 2))
...
# 0.8 Qvalue vs FDR圖 ####
png("C:/Peter/repeatSNP_clumping_FIXpeople/r2_filter_0.8/outcome/FDR_qvalue_r2_0.8_original.png", width = 2000, height = 1000, res = 200)
par(mfrow = c(1, 2))
...
# --- 繪圖 (合併到同一頁)
png("C:/Peter/repeatSNP_clumping_FIXpeople/r2_filter_0.8/outcome/BHplot_r2_0.8_scale.png", width = 2000, height = 1000, res = 200)
par(mfrow = c(1, 2))
...
# 設定 x,y 軸範圍
cutoff <- 0.05
file_name <- paste0("C:/Peter/repeatSNP_clumping_FIXpeople/r2_filter_0.8/outcome/FDR_qvalue_r2_0.8_scale_", cutoff, ".png")
