...
pvalue_0.05_N <- pvalue_0.05_N+ ds %>% filter(`p-value`<0.05) %>% nrow()
...
pvalue_0.05_T <- pvalue_0.05_T+ ds %>% filter(`p-value`<0.05) %>% nrow() 
...
file_name <- c("maf_ds_N_MOSTpvalue",
               "maf_ds_T_MOSTpvalue",
               "maf_gt_N_MOSTpvalue",
               "maf_gt_T_MOSTpvalue")

file_name_all <- c("maf_gt_N_pvalue",
               "maf_gt_T_pvalue")
...
for (i in file_name) {
  compute_FDR(paste0("D:/oral_cancer/expression/trash/",i,".txt"),
...
exp_diff_afQN <- Ordered_normal_quantile(as.matrix(exp[,43:82]), by.row = F) %>% as.data.table() - Ordered_normal_quantile(as.matrix(exp[,3:42] ), by.row = F) %>% as.data.table()
...
file_name <- c("maf_gt_N_pvalue_FDR",
               "maf_gt_T_pvalue_FDR",
...
pvalue_oqn <- pvalue_oqn[,c("maf_gt_N_pvalue_FDR_sigCis-SNP_number","maf_gt_T_pvalue_FDR_sigCis-SNP_number",
                            ...
                            "maf_ds_T_MOSTpvalue_FDR_sigCis-SNP_number") := NULL]
...
file_name <- c("D:/oral_cancer/expression/trash/maf_gt_N_pvalue_FDR",
...
file_name_new <- c("D:/oral_cancer/expression/outcome/multiple_nucleotide_variant/maf_gt_N_pvalue_FDR",
...
file_name <- c("D:/oral_cancer/expression/trash/maf_gt_N_pvalue.txt",
...
file_name <- c("D:/oral_cancer/expression/trash/maf_gt_N_MOSTpvalue.txt",
...
file_name <- c("maf_ds_N_MOSTpvalue_FDR.txt",
