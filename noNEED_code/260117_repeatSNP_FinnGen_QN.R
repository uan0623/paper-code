# ============================================================
# TITLE: Repeat SNP FinnGen QN
# SUBTITLE: Match repeated SNPs with FinnGen data using 1000G/hg19 references.
# SEARCH TAGS: FinnGen, repeat SNP, QN, 1000G, hg19, allele matching
...
# 轉換版本各種錯誤解釋，我們的是 Deleted in new
# <https://genome.ucsc.edu/FAQ/FAQdisplay.html#display4>
...
### Fin hg38 -> hg19 ----
# 把C3_ORALCAVITY_EXALLC 檔案 chr1:123 -> 1:123
...
# hg38 SNP
# finngen(GRCH38/hg38); our eQTL(hg18)，以下轉成  hg38 SNP 操作，因為finngen snp較多

# 13518 snp in hg18 (our eQTL)
# -> 13516 snp in hg38 (our eQTL), "1:148546405", "1:148567630" 轉失敗
# -> 9421 snp in hg38 
...
# MixFinngenPval_7_FIN.txt
# 先複製一份 "D:/oral_cancer/expression/outcome/multiple_nucleotide_variant/QN_maf_gt_N_pvalue_FDR_info.txt" 到
# "C:/Peter/QN_before_eQTL/trash/QN_maf_gt_N_pvalue_FDR_info.txt"
finngen <- fread("D:/oral_cancer/TWAS/summary_stat/FinnGen/C3_ORALCAVITY_EXALLC")
gt_N <- fread("C:/Peter/QN_before_eQTL/trash/QN_maf_gt_N_pvalue_FDR_R2filter_0.8.txt")
...
fwrite(data.table(a=paste0("chr",gt_N$CHR),
       ...
       "C:/Peter/snp_repeat_Finngen/process_trash/snp_r2filter.txt",
         row.names = F, col.names = F, sep = "\t")


# 檔案 snp_r2filter.txt 放進 leftover 轉換成 snp_r2filter_success.bed
a <- fread("C:/Peter/snp_repeat_Finngen/process_trash/snp_r2filter_success.bed")
...
index_hg38_suc <- ! gt_N$SNP %in% c("1:148546405", "1:148567630")
...
fwrite(repeat_snp,
       "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_maf_gt_N_pvalue_FDR_repeatSNP.txt",
         row.names = F, col.names = T, sep = "\t")
...
fwrite(gt_N,
       "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_maf_gt_N_pvalue_FDR_MixFinngenPval.txt",
         row.names = F, col.names = T, sep = "\t")


# 共 13516 snp 轉換成 hg38成功，NA不算
uniqueN(gt_N$snp_hg38, na.rm = T)
# 6400


# 共 9421 snp 有對到 finngen_pval，NA不算
...
gt_N <- fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_maf_gt_N_pvalue_FDR_MixFinngenPval.txt")
...
for(chr_i in chr_list){
  ...
  # 存檔
  fwrite(finngen_chr,
         sprintf("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_gt_N_ALLcisSNP/finngen_cisSNP_gt_N_chr%s.txt", chr_i),
         sep = "\t",
         row.names = FALSE,
         col.names = TRUE)
...
for (i in 1:22) {
  overlaps <- sprintf("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_gt_N_ALLcisSNP/finngen_cisSNP_gt_N_chr%d.txt", i) %>%
    fread() 
  ...
  fwrite(overlaps,
         sprintf("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_gt_N_ALLcisSNP/finngen_cisSNP_gt_N_chr%d.txt", i),
         sep = "\t",
         row.names = FALSE,
         col.names = TRUE)
}
...
for (chr_i in 1:22) {
  ...
  a <- sprintf("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_gt_N_ALLcisSNP/finngen_cisSNP_gt_N_chr%d.txt", chr_i) %>% 
    fread()
...
fwrite(df,
       "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_gt_N_MOSTsnp/finngen_sigSNP_chr1-22.txt",
        row.names = F, col.names = T, sep = "\t")
...
fwrite(df_most,
       "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_maf_gt_N_pvalue_FDR_MixFinngenPval_5.txt",
        row.names = F, col.names = T, sep = "\t")
...
# finngen 裡有一個snp 不同 alt,pval 的情況，對重複snp 取 pval最小的
setorder(finngen, pval)
finngen_unique <- finngen[, .SD[1], by = hg38_snpID ]
fwrite(finngen_unique,
       "D:/oral_cancer/snp_repeat_Finngen/outcome/C3_ORALCAVITY_EXALLC_2_SNPunique.txt",
        row.names = F, col.names = T, sep = "\t")
...
df_most <- fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_maf_gt_N_pvalue_FDR_MixFinngenPval_5.txt")
...
fwrite(data.table(a=paste0("chr",MOSTpval_info$CHR),
       ...
       "C:/Peter/QN_before_eQTL/r2_filter/trash/QN_MixFinngenPval_5_SNPhg38.txt",
         row.names = F, col.names = F, sep = "\t")


# 把 MOSTpval_info$snp_hg38 轉成 hg19 ，檔案 MixFinngenPval_5_SNPhg38.txt 放進leftover轉換成 MixFinngenPval_5_SNPhg38_success.bed
a <- fread("C:/Peter/QN_before_eQTL/r2_filter/trash/QN_MixFinngenPval_5_SNPhg38_success.bed")
...
fwrite(MOSTpval_info,
       "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_gt_N_MOSTsnp/QN_maf_gt_N_pvalue_FDR_MixFinngenPval_6.txt",
        row.names = F, col.names = T, sep = "\t")
...
most_pval_info <- fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_gt_N_MOSTsnp/QN_maf_gt_N_pvalue_FDR_MixFinngenPval_6.txt")
...
fwrite(data.table(k = not_in_finngen$gt_N_hg19),
       "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/not_in_finngen_hg19SNP.txt",
       row.names = F, col.names = F, sep = "\n") 


### combine ----

gt_N_MOSTsnp <- fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_gt_N_MOSTsnp/finngen_sigSNP_chr1-22.txt")
...
fwrite(a,
       "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_maf_gt_N_pvalue_FDR_MixFinngenPval_2.txt",
        row.names = F, col.names = T, sep = "\t")
...
### 1000G EUR ----
# 因為finngen 裡有一個snp 不同 alt,pval 的情況，所以另外建新的檔案
# (outcome/maf_gt_N_pvalue_FDR_MixFinngenPval.txt, 同名加後綴_2, _3 等檔案挑的是，pval 最小的 )

# finngen(hg38); our eQTL(hg18);1000G EUR snp (hg19)，  因為 1000G EUR snp (hg19) 較多，一致轉成hg19

# finngen 20913366 snp 21325073 pval in hg38 (956852 row hg19 =NA)
# -> 19981182 snp success trans from hg38 to hg19
# 19981182 snp 中，有 620 row 是 chrX, chrY;
# 8781 row 是 chr{染色體編號}\_{序列編號}\_random、chrUn\_{序列編號}



# our eQTL 12024 snp hg18
# -> 12024 snp in hg19

# * finngen 資料有原本沒有chrx，轉成hg19後有些變成 chrx
# * 1000G EUR snp 有 31229 snp 重複出現

### Pre-processing ----

# set function run_cmd 這樣plink1.9 執行出錯，就會停下來
...
### 1000G data -> binary file ----
# 因為是 finngen OC GWAS，用 EUR 1000G data
...
gt_N <- fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_maf_gt_N_pvalue_FDR_MixFinngenPval_2.txt")
...
fwrite(data.table(kk = a$hg19_snpID %>% 
                   unique()),
       "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_intersect_finngen_1000G/QN_eQTL_SNPhg19.txt",
       row.names = F, col.names = T, sep = "\t")


# 3. 計算 LD (新)  
#   --list-all 這會列出所有計算過程，即使 r2 無法計算
  
# plink1.9 cmd 不會計算自己跟自己的 LD
# 1. 重新宣告環境變數（確保目前視窗讀得到）
# export EQTL_LIST="/mnt/c/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/not_in_finngen_hg19SNP.txt"
...
#     --ld-window-r2 0 \
#     --list-all \
#     --out "/mnt/c/Peter/QN_before_eQTL/r2_filter/trash/no_intersect_LD/EUR_LDchr${chr}" &
#     
#     if [[ $(($chr % 3)) -eq 0 ]]; then
#         wait
#     fi
...
ld_list <- vector("list", 21)


for (i in c(1:19,21,22)) {
  a <-  sprintf("C:/Peter/QN_before_eQTL/r2_filter/trash/no_intersect_LD/EUR_LDchr%d.ld", i) %>%
    fread(header = T)
...
fwrite(data.table(k = setdiff(not_in_finngen$gt_N_hg19, df_most$gt_N_hg19 )),
       "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/not_in_finngen_1000gEUR_hg19SNP.txt",
       row.names = F, col.names = F, sep = "\n") 
...
fwrite(a,
       "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_MixFinngenPval_7_EUR.txt",
        row.names = F, col.names = T, sep = "\t")
...
k <- fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_MixFinngenPval_7_EUR.txt")
...
### 新增 MOST_snp_nearest_hg19 ----
# 加上 chr
a <-fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_MixFinngenPval_7_EUR.txt")
...
fwrite(data.table(a=paste0("chr",most_snp$chr),
                  b=most_snp$pos, 
                  c=most_snp$pos ) ,
       "C:/Peter/QN_before_eQTL/trash/most_snp.txt",
       row.names = F, col.names = F, sep = "\t")

# 把 MOST_snp_nearest_hg38 轉成 hg19 ，檔案 most_snp.txt 放進leftover轉換成  C:/Peter/QN_before_eQTL/r2_filter/trash/most_snphg19.bed
snp_hg19 <- fread("C:/Peter/QN_before_eQTL/r2_filter/trash/most_snphg19.bed",header=F)
...
eur <- fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_MixFinngenPval_7_EUR.txt")
...
#### choose FIN ----
# <https://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502/> 網站上有兩個紀錄樣本種族的檔案
# 1. integrated_call_samples_v3.20250704.ALL.ped.txt
# 2. integrated_call_samples_v3.20130502.ALL.panel.txt

# 試過兩個檔案，各自選 FIN sample，跑以下算 LD，結果是一樣的
...
#         --ld-window-r2 0 \
#         --list-all \
#         --out "/mnt/c/Peter/QN_before_eQTL/r2_filter/trash/no_intersect_LD/FIN/FIN_LDchr${chr}" &
#     
#     if [[ $(($chr % 3)) -eq 0 ]]; then
#         wait
#     fi
...
for (i in c(1:19,21,22)) {
  a <-  sprintf("C:/Peter/QN_before_eQTL/r2_filter/trash/no_intersect_LD/FIN/FIN_LDchr%d.ld", i) %>%
    fread(header = T)
...
fwrite(data.table(k = setdiff(not_in_finngen$gt_N_hg19, df_most$gt_N_hg19 )),
       "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/not_in_finngen_1000gFIN_hg19SNP.txt",
       row.names = F, col.names = F, sep = "\n") 
...
fwrite(data.table(a=paste0("chr",most_snp$chr),
                  b=most_snp$pos, 
                  c=most_snp$pos ) ,
       "C:/Peter/QN_before_eQTL/r2_filter/trash/most_snp_test.txt",
       row.names = F, col.names = F, sep = "\t")

# 把 MOST_snp_nearest_hg38 轉成 hg19 ，檔案 most_snp.txt 放進leftover轉換成  C:/Peter/QN_before_eQTL/r2_filter/trash/most_snphg19.bed
snp_hg19 <- fread("C:/Peter/QN_before_eQTL/r2_filter/trash/most_snphg19_test.bed",header=F)
snp_hg19[,chr := str_remove(V1, "chr")]
...
fwrite(a,
       "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_MixFinngenPval_7_FIN.txt",
        row.names = F, col.names = T, sep = "\t")
...
# 加上 chr
a <-fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_MixFinngenPval_7_FIN.txt")
all(str_extract(a$gt_N_hg38, ".*(?=:)")==str_extract(a$gt_N_hg19, ".*(?=:)"))
a[,CHR:= str_extract(gt_N_hg38, ".*(?=:)") %>% as.numeric()]
...
# 把 MOST_snp_nearest_hg38 轉成 hg19 ，檔案 most_snp.txt 放進leftover轉換成  C:/Peter/QN_before_eQTL/trash/most_snphg19.bed
snp_hg19 <- fread("C:/Peter/QN_before_eQTL/r2_filter/trash/most_snphg19.bed",header=F)
snp_hg19[,chr := str_remove(V1, "chr")]
...
k <- fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_MixFinngenPval_7_FIN.txt")
...
eur <- fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_MixFinngenPval_7_FIN.txt")
...
# FOR /L %i IN (1,1,22) DO ( plink --bfile "C:/Peter/PCA_1000G_20130502/FIN_sample/trash/deal_repeatSNP/chr_%i_rename" --extract "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/not_in_finngen_1000gFIN_hg19SNP.txt" --freq --out "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/frq/FIN_chr_%i")


# FOR /L %i IN (1,1,22) DO ( plink --bfile "C:/Peter/PCA_1000G_20130502/trash/deal_repeatSNP/chr_%i_rename" --extract "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/not_in_finngen_1000gEUR_hg19SNP.txt" --freq --out "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/frq/EUR_chr_%i")
...
### combine ----
file_paths <- list.files(path = "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/frq/", 
...
fwrite(all_maf, "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/frq/all_EUR_combined.txt", sep = "\t")
...
fwrite(all_maf, "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/frq/all_FIN_combined.txt", sep = "\t")


a <- fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/frq/all_EUR_combined.txt")
k <- fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/not_in_finngen_1000gEUR_hg19SNP.txt",header=F)
setdiff(a$SNP,k$V1)
setdiff(k$V1,a$SNP)


gt_dt <- fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/not_in_finngen_1000gEUR_hg19SNP.txt",header=F)
names(gt_dt) <- c("snp")

gt_dt[,pos:= str_extract(snp, "(?<=\\:)\\d+") %>% as.numeric()]
gt_dt[,chr:= str_extract(snp, ".*(?=:)") %>% as.numeric()]

ld_dt <- fread("C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_MixFinngenPval_7_EUR.txt")
...
# 有 LD 的 26 snp，其中"2:98080989" "6:26722543" 沒紀錄在 1000G ，從code setdiff(k$V1,a$SNP) 得知
gt_dt[has_LD_within_1MB == TRUE]
# 沒有 LD 的 6 snp
gt_dt[has_LD_within_1MB == FALSE]

fwrite(gt_dt,
       "C:/Peter/QN_before_eQTL/r2_filter/outcome/QN_not_in_finngen/1000gEUR_LD_ORnot.txt",
        row.names = F, col.names = T, sep = "\t")
