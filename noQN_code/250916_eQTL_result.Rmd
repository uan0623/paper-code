...
### DS
不設置pvalue門檻，所有pvalue畫圖，分成不同染色體
- Normal part DS autosome, pvalue < 0.05 association number:  29957075
- Tumor part DS autosome, pvalue < 0.05 association number:  30094074



ds_N_pvalue.txt, ds_T_pvalue.txt各自是53GB，太大。
```{r}
# DS ####
pvalue_0.05_N <- 0 
pvalue_0.05_T <- 0
...
### GT 
- Normal part GT autosome, pvalue < 0.05 association number:  6687381
- Tumor part GT autosome, pvalue < 0.05 association number:  6587185  
```{r}
...
### GT merge chr1-22
generate maf_gt_T_pvalue.txt
對於 Each Probe Choose the Most Significant Cis-eQTL，同樣pvalue 大小，篩選距離最近的

- Normal part GT autosome & 0.05<MAF, pvalue < 0.05 association number:  4684348
- Tumor part GT autosome & 0.05<MAF, pvalue < 0.05 association number:  4591259
...
### DS merge chr1-22 
1. MAF>0.05，不設置pvalue門檻，所有pvalue畫圖，分成不同染色體
2. 對於 Each Probe Choose the Most Significant Cis-eQTL，同樣pvalue 大小，篩選距離最近的


Normal part DS autosome & 0.05<MAF, pvalue < 0.05 association number:  4690203
Tumor part DS autosome & 0.05<MAF, pvalue < 0.05 association number:  4593631
```{r}
...
# FDR, Bonfi 
generate maf_gt_N_pvalue_FDR.txt
maf_ds_N_MOSTpvalue 這些eQTL的FDR 結果，會是錯的，因為eQTL分染色體跑，FDR 要chr1~22一起算

1. 重算FDR，挑出 FDR<0.05 的 pairs
2. 針對這些 probe-snp pairs 調整 expression ( quantile normalization )

we set $\lambda=0.7$
$\hat{\pi}_0(\lambda) = min(max(\dfrac{\sum I(p_i \geq \lambda|\ i = 1,\ \cdots ,\ m)}{m(1-\lambda)},0),1)$
...
generate exp_different.txt

- 固定expression，按照不同人的 expression 大小排序，假設每個expression 不同樣本的分佈相似
- Normal, tumor 分別做 Ordered Quantile normalization，轉成 N(0,1) quantile 再相減做 t test
- Normal-Tumor

- 15513 個gene pval_OQN <0.05; 15300 個gene pval_QN <0.05 
- 轉換前做 t test,15581 個gene 有顯著差異
```{r}
...
### use
generate maf_gt_N_pvalue_FDR_info.txt
```{r}
...
刪掉REF, ALT 不是 a,t,c,g 的 rsID，對錯了
```{r}
...
# kai 相比
跟 kai Normal part eQTL result ，我少了 5535481 snp
