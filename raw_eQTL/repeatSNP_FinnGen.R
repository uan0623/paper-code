...
gt_N <- sprintf(
  "C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_maf_gt_N_pvalue_FDR_R2_%s.txt",
...
fwrite(data.table(a=paste0("chr",gt_N$CHR),
       ...
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/trash/snp_r2filter_%s.txt",r2_threshold,r2_threshold),
...
cmd <- paste(
  ...
  sprintf("/mnt/c/Peter/rawData_eQTL/r2_filter_%s/trash/snp_r2filter_%s.txt",r2_threshold,r2_threshold),
  "/mnt/d/oral_cancer/leftover/hg18ToHg38.over.chain",
  sprintf("/mnt/c/Peter/rawData_eQTL/r2_filter_%s/trash/snp_r2filter_hg18TOhg38.bed",r2_threshold) ,
  sprintf("/mnt/c/Peter/rawData_eQTL/r2_filter_%s/trash/hg18TOhg38_unmapped.bed",r2_threshold)
)

system2("wsl", cmd)

a <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/trash/snp_r2filter_hg18TOhg38.bed",r2_threshold) %>% 
  fread(header = F)
# 忽略轉失敗 .bed 檔案的 #Deleted in new 該行

unmapped_path <- sprintf(
  "C:/Peter/rawData_eQTL/r2_filter_%s/trash/hg18TOhg38_unmapped.bed",
  r2_threshold
)
...
fwrite(gt_N,
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_maf_gt_N_pvalue_FDR_MixFinngenPval.txt",r2_threshold),
       row.names = F, col.names = T, sep = "\t")
...
### Overlap ----
gt_N <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_maf_gt_N_pvalue_FDR_MixFinngenPval.txt",r2_threshold) %>% fread()
...
# 刪掉舊檔，避免數量錯誤
# old_files <- sprintf(
#   "C:/Peter/rawData_eQTL/r2_filter_%s/outcome/finngen_cisSNP_gt_N_chr%d.txt",
#   r2_threshold, 1:22
# )
...
for(chr_i in chr_list){
  ...
  # 存檔
  fwrite(finngen_chr,
         sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/finngen_cisSNP_gt_N_chr%s.txt",r2_threshold, chr_i),
         sep = "\t",
         row.names = FALSE,
         col.names = TRUE)
...
for (i in 1:22) {
  file_path <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/finngen_cisSNP_gt_N_chr%d.txt",r2_threshold, i)
  ...
  fwrite(overlaps,
         sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/finngen_cisSNP_gt_N_chr%d.txt",r2_threshold, i),
         sep = "\t",
         row.names = FALSE,
         col.names = TRUE)
}
...
for (chr_i in 1:22) {
  file_path <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/finngen_cisSNP_gt_N_chr%d.txt",r2_threshold, chr_i)
...
fwrite(df,
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/finngen_sigSNP_chr1-22.txt",r2_threshold),
       row.names = F, col.names = T, sep = "\t")
...
fwrite(df_most,
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_maf_gt_N_pvalue_FDR_MixFinngenPval_5.txt",r2_threshold),
       row.names = F, col.names = T, sep = "\t")
...
df_most <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_maf_gt_N_pvalue_FDR_MixFinngenPval_5.txt",r2_threshold) %>% fread()
...
fwrite(data.table(a=paste0("chr",MOSTpval_info$CHR),
       ...
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/trash/raw_MixFinngenPval_5_SNPhg38.txt",r2_threshold),
       row.names = F, col.names = F, sep = "\t")
...
cmd <- paste(
  "/mnt/d/oral_cancer/leftover/liftOver",
  sprintf("/mnt/c/Peter/rawData_eQTL/r2_filter_%s/trash/raw_MixFinngenPval_5_SNPhg38.txt",r2_threshold),
  "/mnt/d/oral_cancer/leftover/hg38ToHg19.over.chain",
  sprintf("/mnt/c/Peter/rawData_eQTL/r2_filter_%s/trash/raw_MixFinngenPval_5_SNPhg38TOhg19_success.bed",r2_threshold) ,
  sprintf("/mnt/c/Peter/rawData_eQTL/r2_filter_%s/trash/SNPhg38TOhg19_unmapped.bed",r2_threshold)
)

system2("wsl", cmd)

a <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/trash/raw_MixFinngenPval_5_SNPhg38TOhg19_success.bed",r2_threshold) %>% 
  fread(header = F)


unmapped_path <- sprintf(
  "C:/Peter/rawData_eQTL/r2_filter_%s/trash/SNPhg38TOhg19_unmapped.bed",
  r2_threshold
)
...
fwrite(MOSTpval_info,
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_maf_gt_N_pvalue_FDR_MixFinngenPval_6.txt",r2_threshold),
       row.names = F, col.names = T, sep = "\t")


# 重新讀取，才會把 MOSTpval_info$alt 從 NA 讀成 ""
MOSTpval_info <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_maf_gt_N_pvalue_FDR_MixFinngenPval_6.txt",r2_threshold) %>% fread()

# 紀錄不在 finngen snp
not_in_finngen <- MOSTpval_info[MOSTpval_info$alt=="",c("gt_N_hg19","gt_N_hg38")]
fwrite(data.table(k = not_in_finngen$gt_N_hg19),
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/not_in_finngen_hg19SNP.txt",r2_threshold),
       row.names = F, col.names = F, sep = "\n") 
...
# 刪掉舊檔案
OUT_DIR <- sprintf(
  "C:/Peter/rawData_eQTL/r2_filter_%s/trash/no_intersect_LD/EUR",
  r2_threshold
)
...
EQTL_LIST <- sprintf(
  "C:/Peter/rawData_eQTL/r2_filter_%s/outcome/not_in_finngen_hg19SNP.txt",
  r2_threshold
)
...
# 刪掉舊檔案
OUT_DIR <- sprintf(
  "C:/Peter/rawData_eQTL/r2_filter_%s/trash/no_intersect_LD/FIN",
  r2_threshold
)
...
gt_N_MOSTsnp <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/finngen_sigSNP_chr1-22.txt",r2_threshold) %>% fread()
...
fwrite(a,
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_maf_gt_N_pvalue_FDR_MixFinngenPval_2.txt",r2_threshold),
       row.names = F, col.names = T, sep = "\t")
...
gt_N <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_maf_gt_N_pvalue_FDR_MixFinngenPval_2.txt",r2_threshold) %>% fread()
...
fwrite(data.table(kk = a$hg19_snpID %>% 
                    unique()),
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_eQTL_SNPhg19.txt",r2_threshold),
       row.names = F, col.names = T, sep = "\t")
...
for (i in 1:22) {
  file_path <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/trash/no_intersect_LD/EUR/EUR_LDchr%d.ld",r2_threshold, i)
...
most_pval_info <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_maf_gt_N_pvalue_FDR_MixFinngenPval_6.txt",r2_threshold) %>% fread()
...
fwrite(data.table(k = setdiff(not_in_finngen$gt_N_hg19, df_most$gt_N_hg19 )),
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/not_in_finngen_1000gEUR_hg19SNP.txt",r2_threshold),
       row.names = F, col.names = F, sep = "\n") 
...
fwrite(a,
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_MixFinngenPval_7_EUR.txt",r2_threshold),
       row.names = F, col.names = T, sep = "\t")
...
# 新增 MOST_snp_hg19 ####
a <-sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_MixFinngenPval_7_EUR.txt",r2_threshold) %>% fread()
...
fwrite(data.table(a=paste0("chr",most_snp$chr),
                  b=most_snp$pos, 
                  c=most_snp$pos ) ,
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/trash/most_snp.txt",r2_threshold),
       row.names = F, col.names = F, sep = "\t")
...
cmd <- paste(
  "/mnt/d/oral_cancer/leftover/liftOver",
  sprintf("/mnt/c/Peter/rawData_eQTL/r2_filter_%s/trash/most_snp.txt",r2_threshold),
  "/mnt/d/oral_cancer/leftover/hg38ToHg19.over.chain",
  sprintf("/mnt/c/Peter/rawData_eQTL/r2_filter_%s/trash/most_SNPhg38TOhg19.bed",r2_threshold) ,
  sprintf("/mnt/c/Peter/rawData_eQTL/r2_filter_%s/trash/hg38TOhg19_unmapped.bed",r2_threshold)
)

system2("wsl", cmd)


snp_hg19 <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/trash/most_SNPhg38TOhg19.bed",r2_threshold) %>% 
  fread(header=F)
snp_hg19[,chr := str_remove(V1, "chr")]


unmapped_path <- sprintf(
  "C:/Peter/rawData_eQTL/r2_filter_%s/trash/hg38TOhg19_unmapped.bed",
  r2_threshold
)
...
k <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_MixFinngenPval_7_EUR.txt",r2_threshold) %>% fread()
...
gt_N <- sprintf(
  "C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_N_bon_R2_%s.txt",
  r2_threshold, r2_threshold
) %>% fread()
...
fwrite(final,
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_MixFinngenPval_7_EUR_bon.txt",r2_threshold),
       row.names = F, col.names = T, sep = "\t") 


cat(sprintf("pass eQTL bon %d snp, retain %d in raw_MixFinngenPval_7_EUR_bon.txt\n", snp_number, nrow(final)))
...
### Merge ----
  ...
  for (i in 1:22) {
    file_path <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/trash/no_intersect_LD/FIN/FIN_LDchr%d.ld", r2_threshold, i)
...
fwrite(data.table(k = setdiff(not_in_finngen$gt_N_hg19, df_most$gt_N_hg19 )),
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/not_in_finngen_1000gFIN_hg19SNP.txt",r2_threshold),
       row.names = F, col.names = F, sep = "\n") 
...
test_eur <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/trash/most_snp.txt",r2_threshold) %>% fread()
...
fwrite(a,
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_MixFinngenPval_7_FIN.txt",r2_threshold),
       row.names = F, col.names = T, sep = "\t")
...
k <- sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_MixFinngenPval_7_FIN.txt",r2_threshold) %>% fread()
...
final <- k[gt_N_hg19 %in% gt_N$hg19_snpID, ] 
fwrite(final,
       sprintf("C:/Peter/rawData_eQTL/r2_filter_%s/outcome/raw_MixFinngenPval_7_FIN_bon.txt",r2_threshold),
       row.names = F, col.names = T, sep = "\t") 



cat(sprintf("pass eQTL bon %d snp, retain %d in raw_MixFinngenPval_7_FIN_bon.txt\n", snp_number, nrow(final)))
